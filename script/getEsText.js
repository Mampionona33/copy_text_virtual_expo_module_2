(() => {
  ...
  chrome.storage.sync.get(['Es_text'], (result) => {
    if (Object.keys(result).length > 0) {
      let prevText = '';
      prevText = result.Es_text;
      ...
      if (prevText.length < 1700) {
        ...
        if (nbTotalText < 1700) {
          chrome.storage.sync.set({ Es_text: outPutText }, () =>
            console.log(outPutText)
          );
        } else {
          alert(`Nombre de caractère pour idp text ES  atteint ${nbTotalText}`);
        }
      }
    } else {
      if (formatList.length > 0) {
        chrome.storage.sync.set({ Es_text: formatList }, () =>
          console.log(outPutText)
        );
      }
      chrome.storage.sync.set({ Es_text: outPutText }, () =>
